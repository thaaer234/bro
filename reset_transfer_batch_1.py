    ...
    try:
        ...
        try:
            ...
            with transaction.atomic():
                ...
                # 8. Reset batch
                print("8. إعادة حالة دفعة الترحيل إلى مسودة...")
                batch.status = 'draft'
                batch.failure_reason = ''
                batch.save()
                
                print("🎉 تم تنظيف حركات الدفعة 1 بنجاح.")
        except AcademicYearTransferBatch.DoesNotExist:
            print("⚠️ لم يتم العثور على دفعة الترحيل رقم 1 (تم تخطي تنظيف حركات الدفعة).")

        # Now proceed to delete all unused general student accounts (1251-000-xxxx) across the DB
        from django.db import transaction
        with transaction.atomic():
            print("🧹 البدء في حذف كافة حسابات ذمم الطلاب العامة المكررة (1251-000-)...")
            ...
            for gen_acc in general_accounts:
                if Transaction.objects.filter(account=gen_acc).count() == 0:
                    gen_acc.delete()
                    deleted_gen_count += 1
            print(f"🎉 تم حذف {deleted_gen_count} حساب ذمة عامة غير مستخدم بنجاح!")
