        ...
        try:
            ...
            for account_id in affected_account_ids:
                try:
                    ...
                    continue
            self.message_user(request, f"تم حذف القيد {obj.reference} بنجاح.", messages.SUCCESS)
        except Exception as e:
            self._log_deletion_debug("delete_model", e)
            error_msg = str(e)
            if "PROTECTED" in error_msg.upper() or "FOREIGN KEY" in error_msg.upper():
                self.message_user(request, "لا يمكن حذف القيد لأنه مرتبط بسجلات أخرى (مثلاً إيصال قبض أو دورة). يجب حذف السجلات المرتبطة أولاً.", messages.ERROR)
            else:
                self.message_user(request, f"فشل الحذف: {error_msg}", messages.ERROR)
        ...
        try:
            ...
            for account_id in affected_account_ids:
                try:
                    ...
                    continue
            self.message_user(request, "تم حذف القيود المختارة بنجاح.", messages.SUCCESS)
        except Exception as e:
            self._log_deletion_debug("delete_queryset", e)
            error_msg = str(e)
            if "PROTECTED" in error_msg.upper() or "FOREIGN KEY" in error_msg.upper():
                self.message_user(request, "فشل حذف بعض القيود لأنها مرتبطة بسجلات أخرى. يرجى حذف القيود واحداً تلو الآخر لمعرفة أيها المسبب للمشكلة.", messages.ERROR)
            else:
                self.message_user(request, f"حدث خطأ أثناء الحذف الجماعي: {error_msg}", messages.ERROR)
...
class StudentReceiptInline(admin.TabularInline):
    model = StudentReceipt
    extra = 0
